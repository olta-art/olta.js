...
##### Property
A field or a key value pair within a document. In our example there is `x` `y` `color` properties for rendering a circle in a location and a given color. Properties are prefaced with `_` and are generated by the contract. Such as `_id` and `_creator` they cannot be changed.

<!-- TODO add links to what metadata properties are available -->
